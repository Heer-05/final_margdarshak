...
- In Render, create a Blueprint and select this repository.
- Render will detect `render.yaml` and create `margdarshak-backend`.
...
- Vercel detects Vite and uses `npm run build`.
- Add environment variable:
    - `VITE_API_URL=https://<your-render-backend>.onrender.com`

`frontend/vercel.json` already includes SPA rewrites, so dashboard routes work on refresh.

### 3. Verify End-to-End

- Open your deployed frontend.
- Upload a resume and check `/health` from frontend network calls.
- In Live Jobs, click Search and confirm status is not `mock`.
